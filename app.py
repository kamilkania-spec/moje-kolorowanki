...
def master_generate(prompt, styl, mode="bw", seed=None, audience="Dzieci", consistency_context=None):
    try:
        # Skupiamy się na estetyce z Twojego drugiego zdjęcia: czyste, cienkie linie, styl liściasty, brak szumu
        quality = (
            "clean white background, sharp thin black lines, professional ink drawing, "
            "minimalist line art, high contrast, 8k resolution, vector style, no noise, "
            "no dithering, no shading, no gray, perfectly smooth contours"
        )
        consistency = f"consistent style with {consistency_context}," if consistency_context else ""
        
        if audience == "Dzieci":
            complexity = (
                "bold clean black outlines, simple whimsical shapes, storybook style, "
                "large coloring areas, charming minimalist details"
            )
        else:
            complexity = (
                "elegant intricate designs, fine smooth lines, professional adult coloring book, "
                "sophisticated composition, botanical and floral leaf patterns"
            )
...
def to_contour_bw(image: Image.Image) -> Image.Image:
    ...
    # 2. Inteligentna binaryzacja (zachowuje cienkie linie z Twojego 2 zdjęcia)
    # Zamiast agresywnego punktu 180, używamy adaptacyjnego podejścia, żeby nie "poszarpać" linii
    bw = gray.point(lambda p: 0 if p < 220 else 255).convert("1")
    
    # 3. Delikatne wygładzenie dla efektu "vector"
    # Zamieniamy na RGB dla Streamlit, bez dodatkowych filtrów psujących jakość
